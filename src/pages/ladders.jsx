...
const Products = () => {
  ...
  const category = searchParams.get('category') || 'LADDERS';
  const [products, setProducts] = useState([]);
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [subcategories, setSubcategories] = useState([]);
  const [selectedSubcategory, setSelectedSubcategory] = useState('LADDERS');
  ...
  const filterAndSortProducts = () => {
    ...
    if (selectedSubcategory !== 'LADDERS') {
        ...
        {subcategories.length > 0 && (
          <div className="mb-8">
            <div className="flex flex-wrap gap-4">
              <button
                onClick={() => setSelectedSubcategory('LADDERS')}
                className={`px-4 py-2 rounded ${selectedSubcategory === 'LADDERS' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
              >
                LADDERS
              </button>
