...
const Products = () => {
  ...
  const category = searchParams.get('category') || 'EAR PROTECTION';
  const [products, setProducts] = useState([]);
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [subcategories, setSubcategories] = useState([]);
  const [selectedSubcategory, setSelectedSubcategory] = useState('EAR PROTECTION');
  ...
  const filterAndSortProducts = () => {
    ...
    if (selectedSubcategory !== 'EAR PROTECTION') {
        ...
        {subcategories.length > 0 && (
          <div className="mb-8">
            <div className="flex flex-wrap gap-4">
              <button
                onClick={() => setSelectedSubcategory('EAR PROTECTION')}
                className={`px-4 py-2 rounded ${selectedSubcategory === 'EAR PROTECTION' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
              >
                EAR PROTECTION
              </button>
