...
const Products = () => {
  ...
  const category = searchParams.get('category') || 'FIRE PROTECTION';
  const [products, setProducts] = useState([]);
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [subcategories, setSubcategories] = useState([]);
  const [selectedSubcategory, setSelectedSubcategory] = useState('FIRE PROTECTION');
  ...
  const filterAndSortProducts = () => {
    ...
    if (selectedSubcategory !== 'FIRE PROTECTION') {
        ...
        {subcategories.length > 0 && (
          <div className="mb-8">
            <div className="flex flex-wrap gap-4">
              <button
                onClick={() => setSelectedSubcategory('FIRE PROTECTION')}
                className={`px-4 py-2 rounded ${selectedSubcategory === 'FIRE PROTECTION' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
              >
                FIRE PROTECTION
              </button>
