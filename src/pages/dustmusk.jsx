...
const Products = () => {
  ...
  const category = searchParams.get('category') || 'DUSTMUSK';
  const [products, setProducts] = useState([]);
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [subcategories, setSubcategories] = useState([]);
  const [selectedSubcategory, setSelectedSubcategory] = useState('DUSTMUSK');
  ...
  const filterAndSortProducts = () => {
    ...
    if (selectedSubcategory !== 'DUSTMUSK') {
        ...
        {subcategories.length > 0 && (
          <div className="mb-8">
            <div className="flex flex-wrap gap-4">
              <button
                onClick={() => setSelectedSubcategory('DUSTMUSK')}
                className={`px-4 py-2 rounded ${selectedSubcategory === 'DUSTMUSK' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
              >
                DUST MUSK
              </button>
