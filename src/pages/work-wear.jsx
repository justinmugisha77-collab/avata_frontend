...
const WorkWear = () => {
  ...
  const category = searchParams.get('category') || 'WORK WEAR';
  const [products, setProducts] = useState([]);
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [subcategories, setSubcategories] = useState([]);
  const [selectedSubcategory, setSelectedSubcategory] = useState('WORK WEAR');
  ...
  const filterAndSortProducts = () => {
    ...
    if (selectedSubcategory !== 'WORK WEAR') {
        ...
        {subcategories.length > 0 && (
          <div className="mb-8">
            <div className="flex flex-wrap gap-4">
              <button
                onClick={() => setSelectedSubcategory('WORK WEAR')}
                className={`px-4 py-2 rounded ${selectedSubcategory === 'WORK WEAR' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
              >
                WORK WEAR
              </button>
