...
const Products = () => {
  ...
  const category = searchParams.get('category') || 'EYE PROTECTION';
  const [products, setProducts] = useState([]);
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [subcategories, setSubcategories] = useState([]);
  const [selectedSubcategory, setSelectedSubcategory] = useState('EYE PROTECTION');
  ...
  const filterAndSortProducts = () => {
    ...
    if (selectedSubcategory !== 'EYE PROTECTION') {
        ...
        {subcategories.length > 0 && (
          <div className="mb-8">
            <div className="flex flex-wrap gap-4">
              <button
                onClick={() => setSelectedSubcategory('EYE PROTECTION')}
                className={`px-4 py-2 rounded ${selectedSubcategory === 'EYE PROTECTION' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
              >
                EYE PROTECTION
              </button>
